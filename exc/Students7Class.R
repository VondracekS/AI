...
# Datasets
titanic = read.csv(file='C:\\Users\\380234\\Desktop\\AI Fin\\Titanic.csv')
zsnew = read.csv(file='C:\\Users\\380234\\Desktop\\AI Fin\\zsnew.csv')
corpdef = read.csv(file='C:\\Users\\380234\\Desktop\\AI Fin\\corpdef.csv')
byty = read.csv(file='C:\\Users\\380234\\Desktop\\AI Fin\\byty_clean.csv')
...
set.seed(300)
# Good loans
tmp.good = idx.good[sample(1:length(idx.good),size=length(idx.bad)*rt,replace=F)] # Undersampling
# The bad loans
...
#####################################
# This takes some time....
ridge.lr = cv.glmnet(x=x,y=y,type.measure='deviance',nfolds=10,family='binomial',alpha=0)
# Check coefficients
coefficients(ridge.lr,s='lambda.min')
p2 = (1/(1+exp(-predict(ridge.lr,newx=newx,s='lambda.min'))))
...
#####################################
# This takes some time....
en.lr = cv.glmnet(x=x,y=y,type.measure='deviance',nfolds=10,family='binomial',alpha=0.5)
# Check coefficients
coefficients(en.lr,s='lambda.min')
p3 = (1/(1+exp(-predict(en.lr,newx=newx,s='lambda.min'))))
f3 = as.factor((p3>thr)*1)
confusionMatrix(f3,reference=ref,positive='1')

#####################################
# The Decision tree - deep
#####################################
...
hist(bagg.pred[200,],breaks=10,xlim=c(0,1))
...
#####################################
# TASK
# Cross-validate mtry = c(3,9,12,24) and max.depth = c(0,6,12)
#####################################
...
#####################################
# TASK
# Estimate LR, EN (cross-validate alpha), RF (cross-validate depth and mtry)
#####################################


#####################################
# TASK - Bonus
# using the dataset with apartments (byty_clean) the price is stored in variable cena
# for some apartments, the price is not given - is this by chance?
#####################################

#####################################
# TASK - Bonus
# 2-step procedure to attack imbalance
# Bastani et al., (2019) Wide and deep learning for peer-to-peer lending
# https://www.sciencedirect.com/science/article/pii/S095741741930377X
#####################################
