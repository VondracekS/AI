...
###################################################################################
setwd("C:/Users/A200083283/OneDrive - Deutsche Telekom AG/non_DT/uni/WS_22/AI")
#######################
# 2.1) OLS models
#######################
oct = read.csv("./data/oct.csv")
ocr = read.csv("./data/ocr.csv")
# OLS models
...
m4 = lm(price ~ km + age + km10 + power_lowest + power_low + power_mid + power_high + power_highest + ba + bb + ke +
  nr + po + tn + tt + petrol + petgas + man + eng1 + eng2 + eng3 + eng4 + eng6, data=ocr)
...
spec = gen.fm(dep='cena',x=NULL)
# Estimate model
m = lm(spec,data=train)
...
# LASSO
library(glmnet)
features = features = c('km','age','km10','power_lowest','power_low','power_mid','power_high',
                        'power_highest','ba','bb','ke','nr','po','tn','tt','petrol',
                        'petgas','man','eng1','eng2','eng3','eng4','eng6')
